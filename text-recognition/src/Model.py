...
class Model: 
	...
	def trainBatch(self, gtTexts, imgs):
		"feed a batch into the NN to train it"
		numBatchElements = len(imgs)
		sparse = self.toSparse(gtTexts)
		rate = 0.01 if self.batchesTrained < 10 else (0.001 if self.batchesTrained < 10000 else 0.0001) # decay learning rate
		evalList = [self.optimizer, self.loss]
		feedDict = {self.inputImgs : imgs, self.gtTexts : sparse , self.seqLen : [Model.maxTextLen] * numBatchElements, self.learningRate : rate, self.is_train: True}
	...
	def inferBatch(self, imgs, calcProbability=False, probabilityOfGT=False):
		...
		feedDict = {self.inputImgs : imgs, self.seqLen : [Model.maxTextLen] * numBatchElements, self.is_train: False}
